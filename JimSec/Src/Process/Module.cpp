...
namespace Process
{
    ...
    /// </summary>
    /// <returns></returns>
    PIMAGE_SECTION_HEADER Module::GetSectionHeaders(PIMAGE_NT_HEADERS nt) const
    {
        if (!nt)
            return nullptr;
    ...
    ULONG Module::GetSectionIndex(PIMAGE_NT_HEADERS nt, PIMAGE_SECTION_HEADER sectionHeader, Enum::Section section)
    {
        if (!nt || !sectionHeader)
            return (ULONG)-1;
